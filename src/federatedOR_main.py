...
if __name__ == '__main__':
    ...
    submodel_dict = {}
    for subset in powerset[:-1]: #exclude the global set 
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        # calculate list of gradients from local_weights old global weights
        gradients = calculate_gradients(local_weights, global_model.state_dict()) 
        
        # update sub-model weights
        for subset in powerset[1:-1]: 
            # calculate the average of the subset of gradients from list of all the local gradients
            avg_gradients_subset = average_weights([gradients[i-1] for i in subset], [fraction[i-1] for i in subset])

            # update the weights of the submodel by adding the avg of the subset of gradients -- OR algorithm
            subset_weights = update_weights_from_gradients(avg_gradients_subset, submodel_dict[subset].state_dict()) 
    ...
    for subset in powerset[:-1]: 
        ...
        # print(f' \n Results after {args.epochs} global rounds of training:') # print check
    ...
    accuracy_file = open('../save/ORfed_{}_{}_{}_{}_{}users.txt'.format(args.dataset, args.model,
    ...
    lines = ['Total Time for Shapley: {0:0.4f}'.format(totalRunTime),
            '\n']
