...
if __name__ == '__main__':
    ...
    for subset in powerset[1:-1]: 
        ...
        for epoch in tqdm(range(args.epochs)):
            ...
            for idx in subset:
                local_model = LocalUpdate(args=args, dataset=train_dataset[idx],
                                        idxs=user_groups[idx], logger=logger)
                w, loss = local_model.update_weights(
                    model=copy.deepcopy(global_model), global_round=epoch, main=0)
                local_weights.append(copy.deepcopy(w))
                local_losses.append(copy.deepcopy(loss))

            # update global weights
            global_weights = average_weights(local_weights, [fraction[i-1] for i in subset]) 
            # print("global_weights is", global_weights) # print check
            ...
            # # print global training loss after every 'i' rounds
            # if (epoch+1) % print_every == 0:
            #     print(f' \nAvg Training Stats for {subset} after {epoch+1} global rounds:')
            #     print(f'Training Loss : {np.mean(np.array(train_loss))}')
        ...
        # print(f' \n Results after {args.epochs} global rounds of training:')
        # # print("|---- Avg Train Accuracy: {:.2f}%".format(100*train_accuracy[-1]))
        # print("|---- Test Accuracy {}: {:.2f}%".format(subset, 100*test_acc))
