...
from utils import get_dataset, average_weights, exp_details, powersettool, shapley, \
                  calculate_gradients, update_weights_from_gradients, error, AdjustedTMC_oneiteration
...
if __name__ == '__main__':
    ...
    num_iterations = 0 # check 
    tolerance = 0.01
    while error(mem_tmc) > err:
        # print("current error:", error(mem_tmc)) # print check 
        num_iterations += 1 # check for TMC
        # print("TMC iteration number: ", num_iterations) # print check
        marginal_contribs = AdjustedTMC_oneiteration(test_acc=test_acc, idxs_users=idxs_users, \
    ...
    accuracy_file = open('../save/AdjustedORTMCTry_{}_{}_{}_{}_{}users.txt'.format(args.dataset, args.model,
